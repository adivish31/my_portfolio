...
export function IntroOverlay() {
  ...
  useEffect(() => {
    const svgEl = document.getElementById('helloIntro') as unknown as SVGSVGElement | null;
    ...
    paths.forEach((p) => {
      ...
      const opDur = (parseFloat(p.dataset.opacityDuration || '') || Math.min(0.5, (parseFloat(p.dataset.duration || '0.6')) * 0.5)) * 1000 * speed;
      const opDelay = (parseFloat(p.dataset.opacityDelay || '') || parseFloat(p.dataset.delay || '0')) * 1000 * speed;
      const len = p.getTotalLength();

      // Stroke animation
      const strokeAnim = p.animate(
        [{ strokeDashoffset: String(len) }, { strokeDashoffset: '0' }],
        { duration, delay, easing: 'cubic-bezier(.4,0,.2,1)' }
      );

      // Opacity animation
      const opacityAnim = p.animate(
        [{ opacity: 0 }, { opacity: 1 }],
        { duration: opDur, delay: opDelay, easing: 'linear' }
      );
  ...
  return (
    <div
      ref={overlayRef}
      className={`intro-overlay ${isHidden ? 'hidden' : ''}`}
      id="introOverlay"
    >
      <svg
        className="hello-svg"
      ...
      >
        ...
        <path
          data-duration="2.8"
          data-delay="0.7"
          data-opacity-duration="0.7"
          data-opacity-delay="0.7"
